...
```
## Read numbers
- One sample (150610_D00125_0239_BC718AANXX_3_IL-TP-013_2) has almost no reads.
- One other sample has < 50 million. All others are >60 million with most being > 100 million
- Exeter samples are mostly > 200 million
...
lengths$Centre<-ifelse(grepl('_R1_',lengths$V1), 'Exeter','Edinburgh')
lengths$Billion_bp <-ifelse(grepl('_R1_',lengths$V1), lengths$Million_reads * 200 / 1000, lengths$Million_reads * 250 / 1000)
hist(lengths$Million_reads, breaks=25)layout(matrix(c(1,2), 2, byrow = TRUE))
p1<-ggplot(lengths, aes(x=Million_reads, fill=Centre))+geom_histogram()+facet_grid(Centre ~ .) + theme(legend.position="none")
p2<-ggplot(lengths, aes(x=Billion_bp, fill=Centre))+geom_histogram()+facet_grid(Centre ~ .) + theme(legend.position="none")
