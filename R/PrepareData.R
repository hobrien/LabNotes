...
read_delim("~/BTSync/FetalRNAseq/LabNotes/Results/BG12_19.txt", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  dplyr::select(SYMBOL, starts_with('norm')) %>% write_tsv("~/BTSync/FetalRNAseq/LabNotes/R/GENEX-FB1/Data/counts.txt")

read_delim("~/BTSync/FetalRNAseq/LabNotes/Results/BG12_19.txt", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  dplyr::select(SYMBOL, Male, Female, log2FoldChange, padj) %>% 
  mutate(ageBin='12-19') %>%
  bind_rows(
