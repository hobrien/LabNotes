...
`r margin_note("- almost all (244 million out of 265 million) now mapping as proper pairs")`
`r margin_note("- the number of non-primary mappings has gone down dramatically (55 million vs 194 million), while the number of uniquely mapped reads has gone up (265 million vs. 226 million)")`

``` {r }
RSeQCstats <- read.delim(paste(folder, ".ex.stats.txt", sep=""), header=FALSE, stringsAsFactors=FALSE, skip=5, sep=':')
...
`r margin_note("- 13 million reads map to rDNA (ca. 5%)")`
`r margin_note("- This is down from 14 million reads, but the number of mappings is up from 120 million to 131 million")`
...
``` {r }
RSeQCstats <- read.delim(paste(folder, ".in.stats.txt", sep=""), header=FALSE, stringsAsFactors=FALSE, skip=5, sep=':')
RSeQCstats[4,2] <-strsplit(RSeQCstats[4,1], ' +')[[1]][4]
RSeQCstats[4,1] <- 'Non primary hits'
colnames(RSeQCstats)<-c('stat', 'result')
RSeQCstats$result <- formatC(as.numeric(RSeQCstats$result), format='d', big.mark=',')
...
``` {r }
RSeQCdist <- read.delim(paste(folder, ".dist.txt", sep=""), header=TRUE, stringsAsFactors=FALSE, skip=4, sep='')
...
```{r }
expt <- read.delim(paste(folder, ".expt.txt", sep=""), skip=3, header=FALSE, sep=':')
...
`r margin_note("- Less than 979,000 of 1,000,000 reads used (up from <2000)")`
`r margin_note("- Shape is unchanged, but a lot smoother")`

```{r include=FALSE}
inner_distance <- read.delim(paste(folder, ".inner_distance_freq.txt", sep=""), header=FALSE)
...
```{r dev.args = list(bg = 'transparent')}
#geneBodyCoverage <- read.delim(paste(folder, ".geneBodyCoverage.txt", sep=""), header=FALSE)
...
```{r }
#junctions <- read.delim(paste(folder, ".junction.txt", sep=""), skip=4, header=FALSE, sep=':')
