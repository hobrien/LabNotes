...
```{r include=FALSE}
#deletion_profile.py, insertion_profile.py, mismatch_profile.py
...
`r margin_note("- Exeter samples appear to have dropped bases throughout, with big peaks in many samples at bases 83 and 84, especially for 16428 and 15533_2")`
`r margin_note("- There is a dip in Q scores at positions 78 and 79 of the unclipped reads, which is 5 bp less than 83-84, but I feel like the corresponding bases should be 88 and 89 in the unclipped reads")`
`r margin_note("- 17922 has a big spike of insertions at the end of read 1")`
`r margin_note("- The Exeter samples also have slightly more SNPs, which I'm guessing is caused by incorrect placement of the gaps in the alignments")`


```{r dev.args = list(bg = 'transparent'), warning=FALSE}
#ggplot(subset(RSeQCmutations, Position > 84 | Position < 83), aes(x=Position, y=Number/ReadNumbers*1000000, group=sample, colour=centre)) + 
#ggplot(subset(RSeQCmutations, sample != 16428 & sample != '15533_2' & sample != 17922 & sample != 16385), aes(x=Position, y=Number/ReadNumbers*1000000, group=sample, colour=centre)) + 
#ggplot(subset(RSeQCmutations, centre !='Exeter' & sample > 18666), aes(x=Position, y=Number/ReadNumbers*1000000, group=sample, colour=sample)) + 
